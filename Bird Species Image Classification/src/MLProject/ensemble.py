...
def count_attribute_numbers(number, c_list):
    ...
    total_count = [0 for col in range(200)]
    labels = attributes[number]
    counts = dict((l, [0 for col in range(200)]) for l in labels)
    for line in c_list:
        #total_count[0] += 1
        #=======================================================================
        # print number
        ...
        # print line[number-1]
        # print counts[0][0]
        # print counts[line[number-1]]
        #=======================================================================
        #counts[int(line[number-1])][0] += 1
        #=======================================================================
        # if line[-1] == -1:                    
        #     counts[int(line[number-1])][1] += 1
        #     total_count[1] += 1
        # else:
        #=======================================================================
        counts[int(line[number-1])][int(line[-1])-1] += 1
        total_count[int(line[-1])-1] += 1
...
def check_example(example, root, gtruth):
    ...
    if root.children[root.attr_val.index(int(example[root.value - 1]))] is None:
        return False
    l = root.children[root.attr_val.index(int(example[root.value - 1]))].value
    
    
    if l == None:
        return False
    else:
        m = root.children[root.attr_val.index(int(example[root.value - 1]))]
...
with open(sys.argv[1].strip(), mode='r') as infile:
...
with open(sys.argv[2].strip(), mode='r') as file1:
...
root = ID3(train_data, list(range(0,312)))

print check_train_and_test_accuracy(root, train_data, test_data, depths)
...
def train_n_trees(train_data, n, m):
    ...
    trees = []
    for i in range(n):
        root = ID3(sset, list(range(0,312)))
        trees.append(root)
    return trees


roots = train_n_trees(train_data, int(sys.argv[3].strip()), 5000)
